...
import { selectFiles, getFilesAsync, createLecturesAsync, updateLecturesAsync } from "../ChatSlice";
import { selectStudents, getAllStudentsAsync } from "../../user/UserSlice";
import { selectLoggedInUser } from "../../auth/AuthSlice.jsx";

const CourseUpsertModal = ({ open, onClose, mode, course }) => {
    ...
    useEffect(() => {
        if (open) {
            ...
            if (mode === 'update' && course) {
                ...
                setSelectedFiles(course.fileIds || []);
    ...
    const handleSubmit = () => {
        ...
        if (mode === 'create') {
            dispatch(createLecturesAsync(courseData));
        } else if (mode === 'update' && course) {
            dispatch(updateLecturesAsync({ id: course._id, data: courseData }));
        }
        ...
        <Modal open={open} onClose={onClose}>
            ...
            >
                <Typography variant="h6" mb={2}>
                    {mode === 'create' ? 'Ders Oluştur' : 'Ders Güncelle'}
                </Typography>
                <Stack spacing={2}>
                    <TextField
                        label="Ders Adı"
